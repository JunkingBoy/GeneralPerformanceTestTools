...
class TokenPool:
    ...
    def _random_token(self) -> str | None:
        with TokenPool.__lock:
        ...
        if not lock_token: return
        with TokenPool.__lock: self._active_pool.discard(chose_username)
        self._e.info("%s 成功取出活跃池用户", LogLabelEnum.SUCCESS.value)
        return self._nosql.get_auth(chose_username)

    def get_access_token(self, timeout: float = 10.0) -> str | None:
        s_time: float = time.time()
        while time.time() - s_time < timeout:
            # 从活跃池中取数据
            with TokenPool.__lock:
                if self._active_pool:
                    res: str | None = self._random_token()
